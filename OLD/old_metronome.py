...
class TransportSlave(object):
    ...
    def __init__(self, inPort, outPort, metronomeSignals):
        self.inPort = inPort
        self.outPort = outPort
        self.metronomeSignals = metronomeSignals
        self._started = False
        self._tick = 0
        self.quarterTick = 0
    ...
    def on_quarter(self):
        print("QUARTER: %s"%self.quarterTick)
        message = self.metronomeSignals[self.quarterTick]
...
if __name__ == '__main__':
    ...
    OUT_PORT = mido.open_output('magenta_clock', virtual=True)
    CC = 1
    METRONOME_SIGNALS  = (
    ...
    ts = TransportSlave(inPort = IN_PORT,outPort = OUT_PORT, metronomeSignals = METRONOME_SIGNALS)
    pp(ts)
    ts.run()
    pp("ciao")
